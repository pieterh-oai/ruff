...
impl ExternalLintRuntime {
    ...
    fn run_on_stmt_with_kind(&self, checker: &mut Checker<'_>, kind: StmtKind, stmt: &Stmt) {
    ...
    fn run_on_expr_with_kind(&self, checker: &mut Checker<'_>, kind: ExprKind, expr: &Expr) {
...
def check_stmt(node, ctx):
    ...
    def visit_call(call):
        current = ctx.semantic.current_expression()
        calls.append((call._kind, current._kind if current else None))

    ctx.semantic.visit(node, expr_Call=visit_call)
    ...
    ctx.report(f"calls={calls};calls_skipped={len(calls_skipped)}")
"#;
...
"#,
        ...
        assert_eq!(
            diagnostics[0].primary_message(),
            "Visit: calls=[('Call', 'Call'), ('Call', 'Call')];calls_skipped=0"
        );
    }
...
"#;
    ...
    #[test]
    fn store_name_resolution_uses_specific_node_binding() {
        let script = r#"
def check_stmt(node, ctx):
    stores = []

    def visit_name(name):
        if name.id == "x" and name.ctx == "Store":
            binding = ctx.semantic.resolve_binding(name)
            stores.append((name._span, binding.range if binding else None))

    only = []

    def visit_only(name):
        if name.id == "y" and name.ctx == "Store":
            binding = ctx.semantic.only_binding(name)
            only.append(binding.range if binding else None)

    only_loads = []

    def visit_only_load(name):
        if name.id == "x" and name.ctx == "Load":
            binding = ctx.semantic.only_binding(name)
            only_loads.append(binding.range if binding else None)

    ctx.semantic.visit(node, expr_Name=visit_name)
    ctx.semantic.visit(node, expr_Name=visit_only)
    ctx.semantic.visit(node, expr_Name=visit_only_load)
    ctx.report(f"stores={stores};only={only};only_loads={only_loads}")
"#;

        let temp = tempdir().unwrap();
        let linter_path = temp.path().join("linter.toml");
        let script_path = temp.path().join("store_resolution.py");
...
"#,
        ...
        assert_eq!(
            diagnostics[0].primary_message(),
            "StoreResolution: stores=[((13, 14), (13, 14)), ((36, 37), (36, 37))];only=[(46, 47)];only_loads=[(13, 14), None]"
        );
    }
}
...
pub(crate) mod semantic {
    ...
    use ruff_python_ast::{Expr, ExprContext, HasNodeIndex, Stmt};
    use ruff_python_semantic::{
        BindingId, BindingKind, ExecutionContext, GeneratorKind, ImportedName, Modules, NodeId,
        NodeRef, ResolvedReference, ResolvedReferenceId, Scope, ScopeId, ScopeKind, Snapshot,
    };
    ...
    impl SemanticModelView {
        ...
        fn visit(
            ...
            let (node_id, root, base_snapshot) = self.with_checker(|checker| {
                let node_id = checked_node_id(node, checker)?;
                let semantic = checker.semantic();
                let root = RootNode::from(*semantic.node(node_id));
                Ok((node_id, root, semantic.snapshot()))
            })?;
            ...
            self.state.restore(base_snapshot)?;
        ...
        fn only_binding(&self, node: &Bound<'_, PyAny>) -> PyResult<Option<PyBinding>> {
            self.with_checker(|checker| {
                ...
                let binding = only_binding_id(checker, node_id);
    ...
    impl From<NodeRef<'_>> for RootNode {
        fn from(node: NodeRef<'_>) -> Self {
            match node {
                NodeRef::Stmt(stmt) => Self::Stmt(ptr::from_ref(stmt)),
                NodeRef::Expr(expr) => Self::Expr(ptr::from_ref(expr)),
            }
        }
    }
    ...
    impl RootNode {
        #[allow(
            unsafe_code,
            reason = "SemanticModelView keeps the checker and AST alive for the active callback."
        )]
        fn walk<'a>(self, visitor: &mut impl SourceOrderVisitor<'a>) {
            match self {
                Self::Stmt(stmt) => source_order::walk_stmt(visitor, unsafe { &*stmt }),
                Self::Expr(expr) => source_order::walk_expr(visitor, unsafe { &*expr }),
            }
        }
    }

    struct PythonCallbackVisitor<'state, 'py> {
        py: Python<'py>,
    ...
    impl PythonCallbackVisitor<'_, '_> {
        fn snapshot_for_node(&self, node: AnyNodeRef<'_>) -> PyResult<Snapshot> {
            let Some(node_id) = node.node_index().load().as_u32().map(NodeId::from_u32) else {
            ...
            self.state.with_checker(|checker| {
                Ok(checker
                    .semantic()
                    .snapshot_for_node(node_id)
                    .unwrap_or(self.base_snapshot))
            })
        }
    ...
    impl<'a> SourceOrderVisitor<'a> for PythonCallbackVisitor<'_, '_> {
        fn enter_node(&mut self, node: AnyNodeRef<'a>) -> TraversalSignal {
            ...
            let projected = match self.state.with_checker(|checker| {
                node_to_python(self.py, checker.locator(), node, self.projection)
            }) {
                Ok(value) => value,
                Err(err) => {
                    self.error = Some(err);
            ...
            let snapshot = match self.snapshot_for_node(node) {
            ...
            if let Err(err) = self.restore(snapshot) {
                self.error = Some(err);
                return TraversalSignal::Skip;
            }
            ...
            let result = match callback.bind(self.py).call1((projected,)) {
                Ok(result) => result,
                Err(err) => {
                    self.error = Some(self.restore(self.base_snapshot).err().unwrap_or(err));
                    return TraversalSignal::Skip;
                }
            };
            if let Err(err) = self.restore(self.base_snapshot) {
                self.error = Some(err);
                return TraversalSignal::Skip;
            }
    ...
    struct SemanticState {
        checker: Cell<Option<*mut Checker<'static>>>,
    }

    impl SemanticState {
        fn new(checker: &mut Checker<'_>) -> Self {
            Self {
                checker: Cell::new(Some(ptr::from_mut(checker).cast::<Checker<'static>>())),
            }
        }
        ...
        #[allow(unsafe_code)]
        fn with_checker<R>(&self, f: impl FnOnce(&Checker<'_>) -> PyResult<R>) -> PyResult<R> {
            let ptr = self.checker_ptr()?;
            let checker = unsafe { &*ptr.cast::<Checker<'_>>() };
            f(checker)
        }

        #[allow(unsafe_code)]
        fn with_checker_mut<R>(
            &self,
            f: impl FnOnce(&mut Checker<'_>) -> PyResult<R>,
        ) -> PyResult<R> {
            let ptr = self.checker_ptr()?;
            let checker = unsafe { &mut *ptr.cast::<Checker<'_>>() };
            f(checker)
        }

        fn checker_ptr(&self) -> PyResult<*mut Checker<'static>> {
            self.checker.get().ok_or_else(|| {
        ...
        fn restore(&self, snapshot: Snapshot) -> PyResult<()> {
            self.with_checker_mut(|checker| {
                checker.semantic_mut().restore(snapshot);
                Ok(())
            })
        }
    }
    ...
    fn only_binding_id(checker: &Checker<'_>, node_id: NodeId) -> Option<BindingId> {
        let semantic = checker.semantic();
        ...
        semantic.only_binding(name).or_else(|| {
            let binding_id = binding_id_for_store(checker, name, node_id)?;
            let binding = semantic.binding(binding_id);
    ...
    fn resolve_binding_id(checker: &Checker<'_>, node_id: NodeId) -> Option<BindingId> {
        ...
        semantic
            .resolve_name(name)
            .or_else(|| binding_id_for_store(checker, name, node_id))
    }
    ...
    fn binding_id_for_store(
        ...
        checker
            ...
            .find_map(|(binding_id, binding)| {
                (binding.source == Some(node_id)
                    && binding.name(checker.source()) == name.id.as_str())
                .then_some(binding_id)
            })
    }
