...
#[cfg(test)]
mod tests {
    use super::generated::{CallExpr, IfExpr};
    use super::source::{SourceFileHandle, with_source_file};
    use super::store::{AstStoreHandle, with_store};
    use super::*;
    use crate::external::ast::target::ExprKind;
    use anyhow::anyhow;
        ...
        Python::with_gil(|py| {
            ...
            with_store(store.clone(), || {
                with_source_file(&source_file, || {
                    let owned_locator = SourceFileHandle::new();
                    let parsed = parse_expression(source).expect("parse expression");
                    ...
                    let call_arguments = node_to_python(
                        py,
                        &locator,
                        AnyNodeRef::from(&expr_call.arguments),
                        projection,
                    )
                    .expect("arguments");

                    let py_call = CallExpr::new_instance(
                        py,
                        ExprKind::Call.as_str().to_string(),
                        span_tuple(py, expr_call.range).unwrap(),
                        source.to_string(),
                        format!("{:?}", &expr_call),
                        store.ensure(&expr_call),
                        store.clone(),
                        call_arguments,
                        None,
                        None,
    ...
    fn if_expr_test_is_eager() {
        Python::with_gil(|py| {
            ...
            let source = "value if cond else fallback";
            let locator = Locator::new(source);
            let source_file = SourceFileBuilder::new("test.py", source).finish();
            let store = AstStoreHandle::new();
            with_store(store.clone(), || {
                with_source_file(&source_file, || {
                    ...
                    let test_value =
                        expr_to_python(py, &locator, expr_if.test.as_ref(), projection)
                            .expect("test expression");

                    let py_if = IfExpr::new_instance(
                        py,
                        ExprKind::If.as_str().to_string(),
                        span_tuple(py, expr_if.range).unwrap(),
                        source.to_string(),
                        format!("{:?}", &expr_if),
                        store.ensure(&expr_if),
                        store.clone(),
                        test_value,
                        owned_locator,
                        projection,
                    )
                    .expect("instantiating IfExpr");
