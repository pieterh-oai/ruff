...
mod tests {
    ...
    fn function_def_projects_decorators() -> anyhow::Result<()> {
        with_python_fixture_result(
            ...
            |py, projection, locator| {
                ...
                let expression = decorators.get_item(0)?.getattr("expression")?;
                let kind: String = expression.getattr("_kind")?.extract()?;
                assert_eq!(kind, "Name");
                Ok(())
            },
        )
    ...
    fn try_stmt_projects_handlers() -> anyhow::Result<()> {
        with_python_fixture_result(
            ...
            |py, projection, locator| {
                ...
                let type_ = handlers.get_item(0)?.getattr("type_")?;
                let kind: String = type_.getattr("_kind")?.extract()?;
                assert_eq!(kind, "Name");
                Ok(())
            },
        )
    ...
    fn comprehension_projects_generators() -> anyhow::Result<()> {
        with_python_fixture_result("[x for x in values]", |py, projection, locator| {
            ...
            let is_async: bool = generators.get_item(0)?.getattr("is_async")?.extract()?;
            assert!(!is_async);
            Ok(())
        })
    }
